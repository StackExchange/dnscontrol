...
func Test_setLabel3Helper(t *testing.T) {
	type args struct {
		short     string
		subdomain string
		origin    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotLabel, gotFQDN, err := setLabel3Helper(tt.args.short, tt.args.subdomain, tt.args.origin)
			if (err != nil) != tt.wantErr {
				t.Errorf("setLabel3Helper() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if gotLabel != tt.short {
				t.Errorf("setLabel3Helper() label = %v, want %v", gotLabel, tt.short)
			}
			if gotFQDN != tt.fqdn {
				t.Errorf("setLabel3Helper() labelFQDN = %v, want %v", gotFQDN, tt.fqdn)
			}
		})
	}
}
