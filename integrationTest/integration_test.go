...
func makeTests(t *testing.T) []*TestGroup {
	...
	tests := []*TestGroup{
		...
		testgroup("R53_ALIAS2",
			...
			tc("create dependent records", a("kyle", "1.2.3.4"), a("cartman", "2.3.4.5")),
			tc("ALIAS to A record in same zone", a("kyle", "1.2.3.4"), a("cartman", "2.3.4.5"), r53alias("kenny", "A", "kyle.**current-domain**")),
			tc("change it", a("kyle", "1.2.3.4"), a("cartman", "2.3.4.5"), r53alias("kenny", "A", "cartman.**current-domain**")),
		),
		...
		testgroup("R53_ALIAS_ORDER",
			...
			tc("create alias+cname in one step",
				r53alias("dev-system", "CNAME", "dev-system18.**current-domain-no-trailing**"),
				cname("dev-system18", "ec2-54-91-33-155.compute-1.amazonaws.com."),
			),
		),
	}
