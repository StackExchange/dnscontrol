...
func Test_graphsort(t *testing.T) {
	...
	t.Run("Deletions in correct order",
		executeGraphSort(
			[]stubRecord{
				...
				{NameFQDN: "example.com", Dependencies: []Dependency{{NameFQDN: "mail.example.com", Type: OLD_DEPENDENCY}}},
			},
			[]string{
				"example.com",
				"mail.example.com",
	...
	t.Run("A Change with dependency on old and new state",
		executeGraphSort(
			[]stubRecord{
				...
				{NameFQDN: "foo.example.com", Dependencies: []Dependency{{NameFQDN: "bar2.example.com", Type: OLD_DEPENDENCY}, {NameFQDN: "new2.example.com", Type: NEW_DEPENDENCY}}},
	...
	t.Run("Reports first and ignore dependencies",
		executeGraphSort(
			[]stubRecord{
				{NameFQDN: "example.com", Dependencies: []Dependency{}},
				{NameFQDN: "test.example.com", Dependencies: []Dependency{{Type: NEW_DEPENDENCY, NameFQDN: "example.com"}}, Type: REPORT},
			},
			[]string{
				"test.example.com",
				"example.com",
	...
	t.Run("Reports are not resolvable as dependencies",
		executeGraphSort(
			[]stubRecord{
				{NameFQDN: "test2.example.com", Dependencies: []Dependency{{Type: NEW_DEPENDENCY, NameFQDN: "test.example.com"}}},
