...
* Free accounts don't have access to `2xlarge` instanace. You'll either need to upgrade your CCI account or change `2xlarge` to `large` in `.circleci/config.yml` in your fork. Please be careful to not include that file when you send a PR. See [https://github.com/StackExchange/dnscontrol/issues/1935](#1935) (Anyone have tips on how to make that easier?)
