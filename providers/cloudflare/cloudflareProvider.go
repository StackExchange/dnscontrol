...
func (c *cloudflareProvider) mkCreateCorrection(newrec *models.RecordConfig, domainID, msg string) []*models.Correction {
	...
	case "PAGE_RULE":
		return []*models.Correction{{
			Msg: msg,
			F:   func() error { return c.createPageRule(domainID, *newrec.AsSingleRedirect().CloudflareRedirect) },
		}}
	case "WORKER_ROUTE":
		return []*models.Correction{{
			Msg: msg,
	...
	case rtypesingleredirect.Name:
		return []*models.Correction{{
			Msg: msg,
			F: func() error {
				return c.createSingleRedirect(domainID, *newrec.AsSingleRedirect().CloudflareRedirect)
			},
		}}
	default:
...
func (c *cloudflareProvider) mkChangeCorrection(oldrec, newrec *models.RecordConfig, domainID string, msg string) []*models.Correction {
	...
	case rtypesingleredirect.Name:
		idTxt = oldrec.AsSingleRedirect().CloudflareRedirect.SRRRulesetID
	default:
	...
	case "PAGE_RULE":
		return []*models.Correction{{
			Msg: msg,
			F: func() error {
				return c.updatePageRule(idTxt, domainID, *newrec.AsSingleRedirect().CloudflareRedirect)
			},
		}}
	case rtypesingleredirect.Name:
		return []*models.Correction{{
...
func (c *cloudflareProvider) mkDeleteCorrection(recType string, origRec *models.RecordConfig, domainID string, msg string) []*models.Correction {
	...
	correction := &models.Correction{
		...
		F: func() error {
			...
			case rtypesingleredirect.Name:
				return c.deleteSingleRedirects(domainID, *origRec.AsSingleRedirect().CloudflareRedirect)
			default:
	...
	for i := len(dc.Records) - 1; i >= 0; i-- {
		...
		if rec.Type == "CF_REDIRECT" || rec.Type == "CF_TEMP_REDIRECT" {
			...
			if c.manageRedirects && !c.manageSingleRedirects {
				...
				TranscodePRtoSR(rec)
				if err := c.LogTranscode(dc.Name, rec.AsSingleRedirect().CloudflareRedirect); err != nil {
					return err
				}
