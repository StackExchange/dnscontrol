...
func newUnifi(m map[string]string, _ json.RawMessage) (*unifiProvider, error) {
	...
	client := newClient(host, consoleID, apiKey, site, skipTLS, debug)
...
func (p *unifiProvider) GetZoneRecords(domain string, meta map[string]string) (models.Records, error) {
	// Fetch all records from UniFi
	allRecords, err := p.client.getRecordsLegacy()
	if err != nil {
	...
	for i := range allRecords {
		...
		if r.Key != domain && !strings.HasSuffix(r.Key, domainSuffix) {
			continue
		}

		rc, err := legacyToRecord(domain, r)
		if err != nil {
			return nil, fmt.Errorf("failed to convert record %s: %w", r.Key, err)
		}
