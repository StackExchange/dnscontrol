...
func (n *CNRClient) createRecordString(rc *models.RecordConfig, domain string) (string, error) {
	...
	case "CAA":
		answer = fmt.Sprintf(`%v %s "%s"`, rc.CaaFlag, rc.CaaTag, answer)
	case "TXT":
	...
	case "SRV":
		...
		// _service._proto.name. TTL Type Priority Weight Port Target.
		// e.g. _sip._tcp.phone.example.org. 86400 IN SRV 5 6 7 sip.example.org.
		answer = fmt.Sprintf("%d %d %d %v", uint32(rc.SrvPriority), rc.SrvWeight, rc.SrvPort, answer)
	default:
...
func (n *CNRClient) isDebugOn() bool {
	// Access the value of the environment variable "MY_VAR"
	value := os.Getenv("CNR_DEBUG")

	// Check if the variable is set. Return false if not set accordingly, otherwise return true.
	return value == "1"
}

// Function for debugging purposes (show API command)
func (n *CNRClient) debugCommand(r *response.Response) error {
	if n.isDebugOn() {
		fmt.Printf("\n----------------------------------------------------------\n")
		fmt.Printf("%s\n", r.GetCommandPlain())
