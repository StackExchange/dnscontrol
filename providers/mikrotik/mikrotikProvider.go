...
func (p *mikrotikProvider) ListZones() ([]string, error) {
	...
	for _, nr := range nativeRecords {
		...
		zone, err := publicsuffix.EffectiveTLDPlusOne(nr.Name)
		if err != nil {
			// Names like "retracker.local" won't match the public suffix list.
			// Fall back to taking the last two labels.
			parts := strings.Split(nr.Name, ".")
			if len(parts) >= 2 {
...
func (p *mikrotikProvider) GetZoneRecordsCorrections(dc *models.DomainConfig, existingRecords models.Records) ([]*models.Correction, int, error) {
	changes, actualChangeCount, err := diff2.ByRecord(existingRecords, dc, fwdCompFunc)
	if err != nil {
		return nil, 0, err
	}
...
// fwdCompFunc returns extra comparison data for MIKROTIK_FWD records so that
// changes to match_subdomain, regexp, and address_list are detected by the
// diff engine.
func fwdCompFunc(rc *models.RecordConfig) string {
	if rc.Type != "MIKROTIK_FWD" || rc.Metadata == nil {
		return ""
	}
	// Include all FWD-specific metadata in a stable order.
	return fmt.Sprintf("address_list=%s match_subdomain=%s regexp=%s",
